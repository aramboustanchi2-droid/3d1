"""
Demo: AI Model Database - Redis Backend
نمایش استفاده از پایگاه داده Redis برای مدل‌های هوش مصنوعی

This demo shows:
- Redis in-memory database
- Microsecond latency access
- JSON serialization
- Key-value + Hash + Sorted Set patterns
- Perfect for real-time predictions
- Same API as SQLite/PostgreSQL/MySQL
...
def demo_redis_database(db):
    """نمایش استفاده از پایگاه داده Redis"""
    print("=== AI Model Database Demo - Redis Backend ===\n")
    print("⚡ In-memory storage with microsecond latency\n")
    ...
    # 8. Log Predictions (Redis is perfect for high-speed inference logging)
    print("\n🔮 Logging predictions...")
    print("   ⚡ Redis excels at real-time prediction logging with microsecond latency")
    for i in range(5):
        db.log_prediction(
            model_version_id=version1_id,
    ...
    print(f"\n✅ Demo completed successfully! Using Redis backend.\n")
    print("💡 Redis Advantages:")
    print("   • Microsecond latency (100x faster than disk-based DBs)")
    print("   • Perfect for real-time inference logging")
    print("   • Excellent for metrics streaming")
    print("   • Optional persistence (RDB + AOF)")
    print("   • Scales horizontally with Redis Cluster")
    print("   • Pub/Sub for real-time monitoring\n")


def main():
    parser = argparse.ArgumentParser(description="AI Model Database Redis Demo")
    ...
    parser.add_argument('--db', type=int, default=0,
                        help='Redis database number 0-15 (default: 0)')
    parser.add_argument('--password', default=None,
                        help='Redis password (if auth enabled)')
    ...
    try:
        ...
        print("   3. Required package is installed (pip install redis)")
